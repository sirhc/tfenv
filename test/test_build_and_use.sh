...
test_build_and_use() {
  # Takes a static version and the optional keyword to build it with
  local k="${2-""}";
  local v="${1}";
  tfenv build "${k}" || return 1;
...
test_build_and_use_with_env() {
  # Takes a static version and the optional keyword to build it with
  local k="${2-""}";
  local v="${1}";
  TFENV_TERRAFORM_VERSION="${k}" tfenv build || return 1;
  check_installed_version "${v}" || return 1;
...
log 'info' '### Test Suite: Build and Use';
...
for ((test_iter=0; test_iter<${tests_count}; ++test_iter )) ; do
  ...
  test_build_and_use_with_env "${v}" "${k}" \
...
log 'info' "## \${HOME}/.terraform-version Test 1/3: Build and Use ( ${v1} )";
test_build_and_use "${v1}" \
  && log info "## \${HOME}/.terraform-version Test 1/1: ( ${v1} ) succeeded" \
  || error_and_proceed "## \${HOME}/.terraform-version Test 1/1: ( ${v1} ) failed";

log 'info' "## \${HOME}/.terraform-version Test 2/3: Override Build with Parameter ( ${v2} )";
test_build_and_use_overridden "${v2}" "${v2}" \
...
log 'info' 'Build invalid specific version';
cleanup || log 'error' 'Cleanup failed?!';
...
for ((test_iter=0; test_iter<${neg_tests_count}; ++test_iter )) ; do
  ...
  [ -z "$(tfenv build "${k}" 2>&1 | grep "${expected_error_message}")" ] \
    && error_and_proceed "Building invalid version ${k}";
done;

if [ "${#errors[@]}" -gt 0 ]; then
  log 'warn' '===== The following build_and_use tests failed =====';
  for error in "${errors[@]}"; do
    log 'warn' "\t${error}";
  done
  log 'error' 'Test failure(s): build_and_use';
else
  log 'info' 'All build_and_use tests passed';
fi;
